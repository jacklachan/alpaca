...
def occ(
    underlying: str = "SPY", exp: date = EXPIRY, right: str = "C", strike: Decimal = Decimal("774")
) -> str:
...
def option_plan(**kw) -> TradePlan:
    base = dict(
        ...
        instrument="option",
        symbol="SPY",
        option_legs=[OptionLeg(symbol=occ(), side="buy", qty=5, limit_price=Decimal("8.00"))],
        side="buy",
        notional_usd=Decimal("4000"),
        max_loss_usd=Decimal("4000"),
...
def test_rejects_option_on_non_allowlisted_underlying():
    v = K.review(
        option_plan(
            ...
            option_legs=[
                OptionLeg(
                    symbol=occ("AAPL", strike=Decimal("230")),
                    side="buy",
                    qty=1,
...
def test_rejects_naked_short_call():
    ...
    plan = option_plan(
        option_legs=[OptionLeg(symbol=occ(), side="sell", qty=200, limit_price=Decimal("8.00"))],
        side="sell",
...
def test_event_trade_uses_its_own_larger_daily_budget():
    ...
    big = option_plan(
        ...
        notional_usd=Decimal("12800"),
        max_loss_usd=Decimal("12800"),
    )
    v = K.review(
        big, state(convex_premium_today=Decimal("7000"), median_order_notional=Decimal("6000"))
    )
    assert v.approved, v.reason
...
def test_long_strangle_is_delta_flat_and_passes_concentration():
    ...
    strangle = option_plan(
        option_legs=[
            OptionLeg(
                symbol=occ(right="C", strike=Decimal("786")),
                side="buy",
                qty=5,
                limit_price=Decimal("6.00"),
            ),
            OptionLeg(
                symbol=occ(right="P", strike=Decimal("762")),
                side="buy",
                qty=5,
                limit_price=Decimal("6.00"),
            ),
        ],
        notional_usd=Decimal("6000"),
        max_loss_usd=Decimal("6000"),
    )
...
def test_rejects_concentrated_directional_option_position():
    fat = option_plan(
        option_legs=[OptionLeg(symbol=occ(), side="buy", qty=100, limit_price=Decimal("0.50"))],
...
def test_rejects_when_core_position_count_is_full():
    positions = [
        Position(symbol=s, instrument="equity", qty=Decimal(10), market_value=Decimal(5000))
        for s in ("SPY", "QQQ", "IWM", "AAPL", "MSFT", "NVDA")
    ]
    v = K.review(equity_plan(symbol="AMZN", stop=Decimal("223")), state(positions=positions))
    assert not v.approved
...
def test_rejects_core_exposure_above_one_times_equity():
    v = K.review(
        equity_plan(notional_usd=Decimal("25000"), max_loss_usd=Decimal("2000")),
...
def test_core_drawdown_trips_kill_switch():
    v = K.review(
        equity_plan(),
        state(core_sleeve_value=Decimal("56000"), core_sleeve_cost_basis=Decimal("60000")),
    )
    assert not v.approved
...
def test_convex_sleeve_going_to_zero_does_not_trip_the_kill_switch():
    ...
    """
    s = state(
        equity=Decimal("89000"),  # convex sleeve wiped, core flat
        core_sleeve_value=Decimal("60000"),
        core_sleeve_cost_basis=Decimal("60000"),
    )
...
def test_rejects_option_expiring_too_soon():
    plan = option_plan(
        option_legs=[
            OptionLeg(
                symbol=occ(exp=date(2026, 9, 4)), side="buy", qty=5, limit_price=Decimal("8.00")
            )
        ]
    )
    v = K.review(plan, state())
...
def test_rejects_option_expiring_too_far_out():
    plan = option_plan(
        option_legs=[
            OptionLeg(
                symbol=occ(exp=date(2026, 10, 16)), side="buy", qty=5, limit_price=Decimal("8.00")
            )
        ]
    )
    v = K.review(plan, state())
...
def test_rejects_fat_finger_notional():
    v = K.review(
        equity_plan(notional_usd=Decimal("20000"), max_loss_usd=Decimal("1600")),
...
def test_rejects_strike_far_from_spot():
    plan = option_plan(
        option_legs=[
            OptionLeg(
                symbol=occ(strike=Decimal("1100")), side="buy", qty=1, limit_price=Decimal("0.05")
            )
        ],
